...
def test():
    ...
    # 1. Sample a random point (x,y) in P256
    ...
    while True:
        ...
        if y_int != None:
            break
    y0, y1 = y_int  # Same x coordinate can have two reflected y coordinates

    # 2. Create two points P0 and P1 with the same x coordinate but different y coordinates
    P0 = Create_P256_point(x, y0)
    P1 = Create_P256_point(x, y1)
    print("P0:", printable_P256_point(P0))
    print("P1:", printable_P256_point(P1))

    # 3. A consistency check: Calculate Q = P0 + P1, which should be the "zero" point of P256 (we normally call it "infinity")
    Q = P0 + P1
    Q_str = printable_P256_point(Q)
    print("P0 + P1 = ", Q_str)

    # 4. Calculate r * P0 (if we use group description, then this is P0^r)
    ...
    # 5. Usage sample of hash_to_P256
    password = urandom(32)  # random password (as byte string)
    hpw_point = hash_to_curve(password)  # Get h(pw), a point in P256
